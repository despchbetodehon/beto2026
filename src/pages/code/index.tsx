...
export default function DashboardCodigos() {
  ...
  const carregarCodigos = async () => {
    try {
      console.log('📥 Carregando códigos de acesso...');
      ...
      for (const docSnap of querySnapshot.docs) {
        ...
        if (data.tipo === 'temporario' && expira && expira <= agora) {
          ...
          console.log('🗑️ Código expirado deletado:', docSnap.id);
        } else {
          ativos.push({ firestoreId: docSnap.id, ...data });
        }
      }
      console.log('📊 Códigos ativos carregados:', ativos.length);
      setCodigos(ativos);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    try {
      const unsubscribeFunc = onSnapshot(
        ...
        (snapshot) => {
          console.log('📡 Dados atualizados do Firestore:', snapshot.docs.length, 'documentos');
          const agora = new Date();
          const ativos: any[] = [];
          ...
          snapshot.docs.forEach((docSnap) => {
            ...
            if (data.tipo === 'temporario' && expira && expira <= agora) {
              deleteDoc(doc(db, 'CodigosDeAcesso', docSnap.id)).catch(err => 
                console.error('Erro ao deletar código expirado:', err)
              );
            } else {
              ativos.push({ firestoreId: docSnap.id, ...data });
            }
          ...
          console.log('✅ Atualizando estado com', ativos.length, 'códigos');
          setCodigos(ativos);
        },
        (error) => {
          console.error('❌ Erro no listener real-time:', error);
          // Fallback: carregar dados estáticos
          carregarCodigos();
          ...
          {codigos.length === 0 && (
            <Box style={{ textAlign: 'center', padding: '40px 20px', background: '#f5f7fa', borderRadius: '12px', marginTop: '20px' }}>
              <Typography variant="h6" style={{ color: '#718096', marginBottom: '20px' }}>
                📭 Nenhum código encontrado ou Firestore em modo offline
              </Typography>
