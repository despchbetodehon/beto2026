...
const ListPost: React.FC<{ setItems: React.Dispatch<React.SetStateAction<Item[]>> }> = ({ setItems }) => {
    ...
    return (
      <Grid item xs={12} md={3} key={numero}>
        <Typography variant="h6" className={classes.sectionTitle}>
          Identificação Do Responsável Legal {numero}
        </Typography>
        <TextField
          name={cpfField}
          label="CPF"
  ...
  const renderSociosText = () => {
    ...
    if (newItem.nomesocio4) socios.push(`seu sócio ${newItem.nomesocio4}`);
    if (newItem.nomesocio5) socios.push(`seu sócio ${newItem.nomesocio5}`);
    ...
    {['ATPV', 'Assinatura', 'Comunicação de Venda'].map((produto) => (
      ...
      >
        {produto}
          ...
          {/* Seletor de Quantidade de Responsáveis Legais */}
          <Grid item xs={12}>
            <Typography variant="h6" className={classes.sectionTitle}>
              Quantidade de Responsáveis Legais
            </Typography>
          ...
          {/* Campos Dinâmicos de Responsáveis Legais */}
